...
def run_chatbot(host, port, embedding_model, generator_model, open_ai_key, summarizer_model):
    ...
    # connect to DB
    db_manager = ChromaManager(host, port, embedding_model)
    ...
    '''
    result = db_manager.add_persona(persona, user_id)
    assert len(result) > 0
    
    # init submodules
    generator = OpenAIGenerator(generator_model, open_ai_key)
    ...
    reulst = db_manager.get_persona_by_user_id(user_id)
    assert len(reulst['documents']) == 0
    
    print('===== FINISH INTEGRATION TEST =====')
    
def test_chatbot(db_host, db_port, embedding_model, generator_model, open_api_key, summarizer_model):
    run_chatbot(db_host, db_port, embedding_model, generator_model, open_api_key, summarizer_model)