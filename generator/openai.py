...
Generate an appropriate response from input.
...
class OpenAIGenerator(Generator):
    ...
    def generate(self, query, persona:str, summary:Union[None, str]=None) -> str:
        if summary is None:
            prompt = chat_prompt_wo_summary.format_prompt(
                persona=persona,
                query=query
            )
        else:
        ...
        return self.model.invoke(prompt.to_messages())
...
def test_open_ai(model_name, openai_api_key):
    ...
    result = generator.generate(query, persona, summary)
